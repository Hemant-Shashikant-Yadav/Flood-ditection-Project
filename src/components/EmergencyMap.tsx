...
import { MapPin, Guitar as Hospital, FireExtinguisher as FireEngine, Slice as Police, School, Building, Bus, AlertTriangle } from 'lucide-react';
...
interface EmergencyLocation {
  ...
  type: 'hospital' | 'fire' | 'police' | 'shelter' | 'school' | 'community' | 'government' | 'transport';
  contact?: string;
  address?: string;
}
...
const EmergencyMap: React.FC = () => {
  ...
  const [floodOverlay, setFloodOverlay] = useState<google.maps.GroundOverlay | null>(null);
  ...
  const createInfoWindowContent = (location: EmergencyLocation) => {
    return `
      <div class="p-4 max-w-xs">
        ...
        ${location.address ? `<p class="text-gray-600 mb-1">${location.address}</p>` : ''}
        ${location.contact ? `<p class="text-blue-600">${location.contact}</p>` : ''}
      </div>
    `;
  };
      ...
      {userLocation && (
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm">Please follow evacuation routes to nearest emergency shelter</p>
          </div>
        </motion.div>
      )}
    </div>
